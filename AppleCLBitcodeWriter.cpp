//===----------- AppleCLBitcodeWriter.cpp - Bitcode Writer ---------------===//
//
//                          SPIR Tools (originally)
//                             AppleCL Encoder
//
...
//===---------------------------------------------------------------------===//
//
// Bitcode writer implementation (LLVM 3.2).
//
// Modified to accept and down-encode a module from LLVM 3.5.
//
//===----------------------------------------------------------------------===//
...
/// type ID.  The value ID that is written is encoded absolute!
static bool PushValueAndType(const Value *V, unsigned InstID,
                             SmallVector<unsigned, 64> &Vals,
                             ValueEnumerator &VE) {
  unsigned ValID = VE.getValueID(V);
  // must be absolute with version 0
...
static void pushValue(const Value *V, unsigned,
                      SmallVector<unsigned, 64> &Vals,
                      ValueEnumerator &VE) {
  Vals.push_back(VE.getValueID(V));
}
...
static void WriteInstruction(const Instruction &I, unsigned InstID,
  ...
  case Instruction::IndirectBr:
    ...
    // must all be absolute with version 0
    for (unsigned i = 0, e = I.getNumOperands(); i != e; ++i)
